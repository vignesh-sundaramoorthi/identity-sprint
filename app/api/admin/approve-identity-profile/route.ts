...
// - BUG-020 fix (Forge H175): also stores snapshot if provided. Snapshot is the
//   highest-trust coaching artifact — written post-call from conversation memory.
//   Unlike label + qualities (re-generatable), snapshot cannot be recovered after the
//   call fades. Must be persisted here (not a separate endpoint).
//
// After approval:
...
export async function POST(req: Request) {
  ...
  let body: { participant_id: string; anchor_statement?: string; snapshot?: string }
  try {
    body = await req.json()
  } catch {
  ...
  const { participant_id, anchor_statement, snapshot } = body
  ...
  // BUG-020: persist snapshot on approve
  if (snapshot && typeof snapshot === 'string' && snapshot.trim()) {
    updatePayload.snapshot = snapshot.trim()
  }
  ...
  return NextResponse.json({
    ...
    snapshot: updatePayload.snapshot ?? null,
