...
// India Trust Note — Craft H511/H513 confirmed copy (above CTA per Craft H513 + SDT autonomy framing)
// INDIA_TRUST_NOTE.line1 = "No ₹3 trial. No auto-charge."
// INDIA_TRUST_NOTE.line2 = "One application. One conversation. You decide."
function IndiaTrustNote() {
  return (
    <div className="bg-green-50 border border-green-200 rounded-xl px-5 py-4 mb-4">
      <p className="text-green-800 text-sm font-semibold">No ₹3 trial. No auto-charge.</p>
      <p className="text-green-700 text-sm mt-0.5">One application. One conversation. You decide.</p>
    </div>
  )
}

export default function ApplyClient({ isIndia }: { isIndia: boolean }) {
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-6 pb-24">
        ...
        <form onSubmit={handleFormSubmit} className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 space-y-7">
          ...
          <div className="pt-2">
            {/* India Trust Note — ABOVE the CTA per Craft H513 (SDT autonomy: trust signal before conversion ask) */}
            {isIndia && <IndiaTrustNote />}
